...
def load_vowel():
    return normalize(torch.from_numpy(np.loadtxt(os.path.join("data", "vowel.dat"), delimiter=',', skiprows=0)).float())

def load_glass():
    X = normalize(torch.from_numpy(np.loadtxt(os.path.join("data", "glass.data"), delimiter=',', skiprows=0)).float())
    X = X[:,1:-1].clone()
    return X

def load_pendigits():
    X = normalize(torch.from_numpy(np.loadtxt(os.path.join("data", "pendigits.tra"), delimiter=',', skiprows=0)).float())
    X = X[:,:-1].clone()
    return X
...
def load_synt(d, n, cl, re, cl_d = None):
    ...
    gen_dir = os.path.join('data', 'gen')
    if not os.path.exists(gen_dir):
        os.makedirs(gen_dir)

    file = os.path.join(gen_dir, name + '.dat')

    if not os.path.isfile(file):
        print("generating new data set")
        # Windows-compatible: Use subprocess instead of os.system for better cross-platform support
        import subprocess
        import sys
        
        cluster_gen_path = os.path.join('data', 'cluster_generator')
        
        # Check if cluster_generator exists
        if not os.path.exists(cluster_gen_path):
            print(f"Warning: cluster_generator not found at {cluster_gen_path}")
            print("The cluster_generator binary is platform-specific. On Windows, you may need to:")
            print("1. Build cluster_generator from source for Windows, or")
            print("2. Use the load_synt_gauss() function instead, which is pure Python")
            raise FileNotFoundError(f"cluster_generator not found at {cluster_gen_path}")
        
        # On Windows, we might need .exe extension
        if sys.platform == 'win32' and not cluster_gen_path.endswith('.exe'):
            cluster_gen_path_exe = cluster_gen_path + '.exe'
            if os.path.exists(cluster_gen_path_exe):
                cluster_gen_path = cluster_gen_path_exe
        
        cmd = [cluster_gen_path, 
               "-n" + str(n+1), 
               "-d" + str(d), 
               "-k" + str(cl_d),
               "-m" + str(cl), 
